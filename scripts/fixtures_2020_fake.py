...
def update_fixtures(request_args):
    ...
    for json_obj in response:
        ...
        fixture_id = int(str(999) + str(json_obj["fixture"]["id"]))
        ...
        blabla = datetime.datetime.fromtimestamp(fixture_timestamp)
        blabla = blabla + datetime.timedelta(weeks=+20)

        fixture['timestamp'] = int(blabla.timestamp())
        fixture['date'] = datetime.datetime.fromtimestamp(fixture['timestamp']).isoformat()
